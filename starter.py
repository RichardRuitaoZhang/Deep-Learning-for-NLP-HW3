...
def finetune_obqa(model,opt,tokenizer):
    ...
    # Implement code required to fine-tune the pre-trained model on the Open Book QA dataset.
    # It may be helpful to implement your own dataloader() for more precise control.  You will 
    # need to calculate to probabsility assigned to each answer and select the answer with the 
    # highest estimated probability as the correct answer.  How you structure your model inputs
    # and outputs is up to you.  I was able to acheive a test accuracy of 36.2% after 18 epochs.
    # You may also find it helpful to implement to softmax manually.  Using a single batch with
    # stochastic gradient descent took about 6 minutes per epoch.
     

def testmodel_obqa(model,opt,tokenizer):
    print('validating....')
    ...
    # Implement code here to evaluate you fine-tuned model on the validation set.  You may want 
    # to use some of the code implemented above for fine-tuning, without performing gradient
    # descent.  It's good practice to measure validation accuracy after each epoch.
